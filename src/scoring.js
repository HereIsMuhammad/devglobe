/**
 * Scoring module — computes composite developer score (0-100)
 */
const Scoring = (() => {
  ...
  // Log-normalize a value: maps [0, max] → [0, 1] using log scale
  ...
  // Linear normalize: maps [0, max] → [0, 1]
  ...
  /**
   * Compute composite score for a single developer given dataset-wide maxes
   */
  ...
  /**
   * Score all developers in the dataset
   */
  function scoreAll(developers) {
    // Compute max values across all developers
    const maxValues = {
  ...
  /**
   * Determine the "platform color" based on which dimensions dominate
   */
  function getPlatformColor(dimensions) {
    ...
    if (githubStrength > soStrength * 3) return '#2ea44f'; // GitHub green
    if (soStrength > githubStrength * 1.5) return '#f48024'; // SO orange
    return '#3b82f6'; // Balanced blue
  }

  return { scoreAll, computeScore, getPlatformColor, WEIGHTS };
})();
